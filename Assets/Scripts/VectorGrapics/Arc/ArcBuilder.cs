...
public class ArcBuilder : MonoBehaviour
{
    ...
    private float _Before_arcRadius;
    private float _Before_maskRadius;
    ...
    public float EndAngleDeg => endAngleDeg;
    ...
    {
        startAngleDeg = _Rotation + degree / -2f;
        endAngleDeg = _Rotation + degree / 2f;
    }
    public void GetRadius(out float arc, out float mask)
    {
        arc = arcRadius;
        mask = maskRadius;
    }
    public void SetRadius(float arc, float mask)
    {
        arcRadius = arc;
        maskRadius = mask;
    }
    ...
    {
        if (IsAlwaysUpdate ||
            _Before_arcRadius  != arcRadius || 
            _Before_maskRadius != maskRadius)
        {
#if UNITY_EDITOR
